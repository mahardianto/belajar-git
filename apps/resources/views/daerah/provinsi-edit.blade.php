...
<section class="content">
  ...
  <div class="box">
    ...
    <div class="box-body">
      ...
      <!-- Error or Success Notif -->
      <div class="col-md-6">
        <form role="form" method="POST" action="{{ url('/provinsi/edit') }}">
        {!! csrf_field() !!}
          <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
            <label for="namaPropinsi">Edit Nama Propinsi</label>
            @foreach ($datum as $provinsi)
             <input type="text" class="form-control" placeholder="Nama Propinsi" name="name" value="{{ $provinsi->name }}" pattern="([a-zA-Z0-9]| |/|\|@|#|$|%|&)+" required>
             @if ($errors->has('name'))
